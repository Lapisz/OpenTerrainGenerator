...
public class OTGNoiseChunkGenerator extends ChunkGenerator {
    ...
    public static List<ConfiguredStructureFeature<?, ?>> noiseAffectingFeatures = Arrays.asList(
            StructureFeatures.VILLAGE_PLAINS.value(),
            StructureFeatures.VILLAGE_DESERT.value(),
            StructureFeatures.VILLAGE_SAVANNA.value(),
            StructureFeatures.VILLAGE_SNOWY.value(),
            StructureFeatures.VILLAGE_TAIGA.value(),
            StructureFeatures.PILLAGER_OUTPOST.value(),
            StructureFeatures.FORTRESS.value(),
            StructureFeatures.BASTION_REMNANT.value()
    );
    ...
    private OTGNoiseChunkGenerator(String presetFolderName, String dimConfigName, BiomeSource biomePopulationSource, BiomeSource biomeRuntimeSource, Registry<StructureSet> structureSetRegistry, Registry<NormalNoise.NoiseParameters> noiseRegistry, long seed, Holder<NoiseGeneratorSettings> dimensionSettingsHolder) {
        ...
        this.internalGenerator = new OTGChunkGenerator(this.preset, seed, (ILayerSource) biomePopulationSource, ((FabricPresetLoader) OTG.getEngine().getPresetLoader()).getGlobalIdMapping(presetFolderName), OTG.getEngine().getLogger());
    ...
    public void buildNoise(StructureFeatureManager manager, ChunkAccess chunk) {
        ...
        if (cachedChunk != null) {
            ...
            for (ConfiguredStructureFeature<?, ?> structure : noiseAffectingFeatures) {
    ...
    public void applyCarvers(WorldGenRegion p_187691_, long seed, BiomeManager biomeManager, StructureFeatureManager structureFeatureManager, ChunkAccess chunk, GenerationStep.Carving stage) {
        ...
        if (stage == GenerationStep.Carving.AIR) {
            ...
            boolean cavesEnabled = this.preset.getWorldConfig().getCavesEnabled() && StreamSupport.stream(list.spliterator(), false)
                    .anyMatch(
                    a -> defaultCaves.stream().anyMatch(
                            b -> b.equals(
                                    Registry.CARVER.getKey(a.value().worldCarver).toString()
                            )
                    )
            );
            List<String> defaultRavines = Arrays.asList("minecraft:canyon", "minecraft:underwater_canyon");
            boolean ravinesEnabled = this.preset.getWorldConfig().getRavinesEnabled() && StreamSupport.stream(list.spliterator(), false)
                    .anyMatch(
...
//	}
    ...
    public void spawnOriginalMobs(WorldGenRegion worldGenRegion) {
        if (!this.generatorSettingsHolder.value().disableMobGeneration()) {
            ...
            NaturalSpawner.spawnMobsForChunkGeneration(worldGenRegion, ((FabricBiome) biome).getHolder(), worldGenRegion.getCenter(), sharedseedrandom);
        }
    }
    ...
    public Boolean checkHasVanillaStructureWithoutLoading(ServerLevel world, ChunkCoordinate chunkCoord) {
        return this.shadowChunkGenerator.checkHasVanillaStructureWithoutLoading(world, this, (OTGBiomeProvider) this.biomeSource, this.possibleStructureSets(), chunkCoord, this.internalGenerator.getCachedBiomeProvider(), false);
    }
